...
def init():
    ...
    c.executemany(
        ...
        [
            ("admin", "1234", "김민수", "경영지원팀", "사원", "070-1234-5678", "admin"),
            ("user1", "1234", "이영희", "디자인팀", "대리", "070-1234-5679", "employee"),
            ("user2", "1234", "박철수", "개발팀", "과장", "070-1234-5680", "employee"),
        ]
    )

    # ===== 업무 =====
    # (user_id, work_date, category, title, details, issues, dept, due_label, status)
    c.executemany(
        "INSERT INTO jobs (user_id, work_date, category, title, details, issues, dept, due_label, status) VALUES (?,?,?,?,?,?,?,?,?)",
        [
            # 진행 중 / 긴급 (emp_dash 할 일 목록)
            ...
            # 완료 12건 (complete_job.html 하드코딩 내용)
    ...
    # ===== AS 요청 =====
    c.executemany(
        "INSERT INTO as_requests (user_id, category, urgency, title, details, status) VALUES (?,?,?,?,?,?)",
        [
            (2, "hardware", "high", "모니터 깜빡임 현상", "업무 중 모니터가 간헐적으로 깜빡입니다. 재부팅 후에도 동일.", "pending"),
            (3, "software", "medium", "ERP 접속 오류", "오전 9시경부터 ERP 페이지 로딩이 되지 않습니다.", "in_progress"),
            (1, "network", "low", "Wi-Fi 속도 저하", "오후 시간대 2층 사무실 네트워크가 느려집니다.", "resolved"),
        ]
    )

    # ===== 게시글 (community 4건 + notice/general/qna 4건) =====
    c.executemany(
        ...
        [
            ...
            (2, "design", "실무에서 자주 쓰는 피그마(Figma) 플러그인 추천 5가지",
             ...
             "박디잔", "UI/UX", 210),
            (1, "sales", "B2B 콜드메일 오픈율 높이는 제목 작성 노하우 질문드립니다 ㅠㅠ",
    ...
    # ===== 메시지 =====
    c.executemany(
        "INSERT INTO messages (user_id, sender, body, time_label, is_read) VALUES (?,?,?,?,?)",
        [
            (1, "이매니저",
    ...
    # ===== ERP 문서 (7종) =====
    # (user_id, doc_type, title, content, dept, due_label, status)
    c.executemany(
